...
const minimumSetCount = 50;
const categoryPlaceholders = coreCategoryIds.map(() => "?").join(", ");

console.log("\n=== MECHANICAL ASSOCIATION STRENGTH ===\n");
console.log(`Minimum pair set count: ${minimumSetCount}\n`);
...
const associations = db.prepare(`
  ...
  ORDER BY lift DESC, set_count DESC
  LIMIT 20;
`).all(...coreCategoryIds, minimumSetCount);
